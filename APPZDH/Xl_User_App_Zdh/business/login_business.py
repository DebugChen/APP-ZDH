...
def XL_User_login(username,password):
    # 后获取封装好的驱动
    dr = appium_desired()
    # # 判断是否有更新提示
    # try:# 尝试运行
    ...
    dr.find_element_by_id(
        'android:id/button1').click()
    # 输入账户
    dr.find_element_by_id('org.xc.peoplelive:id/etTel').send_keys(username)
    # dr.find_element_by_xpath('//android.widget.EditText[@text="请输入手机号"]').send_keys(username)
    #dr.find_element_by_xpath('//*[@class="android.widget.EditText"and@text="请输入手机号"]').send_keys(username)
    # 输入验证码
    dr.find_element_by_id('org.xc.peoplelive:id/etCode').send_keys(password)
    # 点击屏幕键盘的搜索键，收起输入法
    dr.press_keycode(66)
    # 点击登录
    ...
    try:
        dr.find_element_by_xpath('//*[@class="android.widget.TextView"and@text="个人"]')
...
if __name__=='__main__':
    XL_User_login("13200000000", "123456")
